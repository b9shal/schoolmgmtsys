...
const { admissionCategory } = require("../models");
...
const validate = [
  ...
  .withMessage("category name should be atleast 1 char and atmost 50 chars long")
]
...
router.get("/list", async function(req, res){
  ...
  try {
    ...
    var status = 200
    const data = await admissionCategory.findAll()
...
router.post("/add", validate, async function(req, res) {
  ...
  try {
    ...
    if(!errors.isEmpty()) {
      ...
      await admissionCategory.create({
        categoryName
      },
      {
        transaction
...
router.delete("/delete/:id", async function(req, res){
  ...
  try {
    ...
    await admissionCategory.destroy({ 
      where: {
        id
      }
    }).catch(err => {
...
router.patch("/edit/:id", validate, async function(req, res){
  ...
  try {
    ...
    if(!errors.isEmpty()) {
      ...
      transaction = await models.sequelize.transaction()
      await admissionCategory.update({categoryName},
      {
        where: { id }
      },
      {
