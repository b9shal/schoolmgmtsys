...
const { employeeAttendance, employee, role } = require("../models");
...
router.get("/list", async function(req, res){
  ...
  try {
    ...
    var status = 200
    const roleId = await parseInt(req.query.id)
    const data = await employeeAttendance.findAll({
      attributes: ["status", "remarks", "date"],
      include: [
        { model: role, attributes: ["type"],
          include: [
            { model: employee, attributes: ["id", "name", "photo"], where: { roleId } },
          ]
        }
      ]
    })
  ...
  try {
    ...
    if(!errors.isEmpty()) {
      ...
      await models.sequelize.queryInterface
        .bulkInsert(models.employeeAttendance.getTableName(),
        employeeAttendanceEntry,
      {
        transaction
      }).catch(err => {
